...
#from BackBaseESN import BaseESN
...
class PredictionESN(BaseESN):
    def __init__(self, n_input, n_reservoir, n_output,
        ...
        self._WOut = np.random.rand(n_output, 1 + n_input + n_reservoir)#bias term
    ...
    def fit(self, inputData, outputData, transientTime="AutoReduce", transientTimeCalculationEpsilon = 1e-3, transientTimeCalculationLength = 20, verbose=0):
        ...
        for i in range(timeseriesCount):
            if inputData is not None:
                xx, yy = self.propagate(inputData[i], outputData[i], transientTime, verbose-1)
                self._X[:, i*partialLength:(i+1)*partialLength] = xx
            else:
                xx,yy = self.propagate(None, outputData[i], transientTime, verbose-1)
                self._X[:, i*partialLength:(i+1)*partialLength] = xx
            if (verbose > 0):
                bar.update(i)
        if (verbose > 0):
    ...
    def predict(self, inputData, continuation=True, initialData=None, update_processor=lambda x:x, verbose=0):
        ...
        X, yy = self.propagate(inputData, verbose=verbose)
